...
init gs =
    Model False 0 (repeat (gs * gs) Dead) gs "" "0" Nothing Nothing
    ...
    case msg of
        ...
        Tick _ ->
            if (.running model) then
                if countNbAlive (.grid model) == 0 then
                    { model | running = False } ! []
                else
                    { model
                        | generation = (.generation model) + 1
        ...
        Random ->
            case .initSeed model of
                Nothing ->
                    { model | error = Just "No random seed" } ! []
...
view model =
    let
        sizeCoordSys =
            toString ((.gridSize model) * 20)

        sizeViewPort =
            toString ((.gridSize model) * 30) ++ "px"
    in
        div [ id "main" ]
            ...
            , case (.error model) of
                ...
                Just e ->
                    p [] [ text <| "Error: " ++ e ]
            , style_ ".cell:hover {fill: #8FBC8F; } input[type=range]{\n                border: 1px solid white;\n                outline: 2px solid white;\n                outline-offset: -1px;\n                }\n                input[type='range']::-moz-focus-outer { border: 0; }"
            ]
        ...
        renderCell n cell =
            ...
            in
                rect
                    ...
                    []
    in
        toList (indexedMap renderCell (.grid model))
...
shuffle xs seed =
    let
        ...
        sortedList =
            sortWith (\( i1, _ ) ( i2, _ ) -> compare i1 i2) (zip indexList xs)
    in
...
randomize grid gs sd n =
    ...
    in
        List.foldr (\p acc -> set p Alive acc) newGrid randIndexes
...
randomize2 grid gs sd n =
    let
        ...
        listGen =
            Random.list (gs * gs) lifeGen
    in
        Tuple.first (Random.step listGen (initialSeed sd)) |> fromList
